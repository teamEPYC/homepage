---
title: "State Model"
description: "This series of blog posts has covered an overview of Miden’s architecture and a deep dive into its transaction model"
...
To return where this began, the state model Miden uses is quite unique, a combination of the account model (as on Ethereum), the UTXO-model (as on Bitcoin), and ZK-proofs. The result is an actor-based model with a concurrent offchain state.
